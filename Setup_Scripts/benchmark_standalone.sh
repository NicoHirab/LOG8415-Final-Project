...
sudo sysbench oltp_read_write --tables=8 --threads=6 --time=360 --max-requests=0 --mysql-db=sakila --mysql-user=root prepare
sudo sysbench oltp_read_write --tables=8 --threads=6 --time=360 --max-requests=0 --mysql-db=sakila --mysql-user=root run > results.txt
